...
class Track(_BaseObject, _Taggable):
    ...
    def scrobble(self, start_time):
        ...
        if album:
            ...
            album_title = ""
        duration = str(int(self.get_duration() / 1000.))
        mbid = self.get_mbid() or ""
...
"""
        params = self._get_params()
        params.update(dict(track=track_title,
                           timestamp=str(int(start_time)),
                           artist=artist_name,
                           album=album_title,
                           mbid=mbid, duration=duration))
        self._request("track.scrobble", False, params)
